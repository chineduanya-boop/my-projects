// Uploads "The God of High School" to the live site (R2 + Supabase)
...
const SOURCE_DIR = 'C:/Users/Administrator/Downloads/Telegram Desktop/The God Of High school';
const COVER_FILE = 'FdTHz2rVQAEVrh1.jpeg';
...
// "Ch - 001 The God Of High School.pdf" → 1
// "Ch - 1 The God Of High School.pdf"   → 1
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/gohs-cover-${Date.now()}.jpeg`,
    'image/jpeg'
  );
  ...
  const existing = await pool.query(
    "SELECT id FROM comics WHERE title = 'The God of High School'"
  );
  if (existing.rows.length) {
    ...
    `, [
      ...
      coverUrl,
      'Finished',
      JSON.stringify(['Action', 'Adventure', 'Comedy', 'Supernatural', 'Manga']),
      0
    ]);
    comicId = r.rows[0].id;
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/gohs-ch${chNum}-${Date.now()}.pdf`;
