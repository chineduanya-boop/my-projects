// Uploads "Pick Me Up, Infinite Gacha" to the live site (R2 + Supabase)
...
const SOURCE_DIR = 'C:/Users/Administrator/Downloads/Telegram Desktop/Pick Me Up, Infinite Gacha';
const COVER_FILE = 'photo_2026-04-04_15-58-18.jpg';
...
// "[CH - 1] Pick Me Up.pdf" → 1
// "[CH - 102] Pick Me Up, Infinite Gacha.pdf" → 102
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/pickmeup-cover-${Date.now()}.jpg`,
    'image/jpeg'
  );
  ...
  const existing = await pool.query(
    "SELECT id FROM comics WHERE title = 'Pick Me Up, Infinite Gacha'"
  );
  if (existing.rows.length) {
    ...
    `, [
      ...
      'Reae',
      'Reae',
      'In the mobile gacha game infamous for its atrocious difficulty, the Master ranked 5th in the world, \'Loki\', loses consciousness while raiding the Dungeon. After waking up, \'Loki\' realizes he has turned into a Level 1, 1-Star Hero—\'Han Yslat\'. To return to Earth, he must lead the newbie Master and heroes and clear the 100th floor of the Dungeon! This is the story of hard carrying by Master Loki who never accepts even a single defeat.',
      coverUrl,
      'Ongoing',
  ...
  for (const file of allFiles) {
    ...
    else console.log(`  DUPLICATE (keeping first): Chapter ${num} — skipping ${file}`);
  }
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/pickmeup-ch${chNum}-${Date.now()}.pdf`;
  ...
  console.log(`View at: https://mangvault.com/pick-me-up-infinite-gacha`);
  await pool.end();
}
