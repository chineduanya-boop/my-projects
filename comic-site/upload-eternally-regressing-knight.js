...
const SOURCE_DIR = 'E:\\New folder\\Telegram Desktop\\Eternally Regressing Knight';
const COVER_FILE = 'eternally-regressing-knight.ef0006-400.webp';
...
// Handles: "[000] Eternally Regressing Knight @Manhwa_Garden.pdf"
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/eternally-regressing-knight-cover-${Date.now()}.webp`,
    'image/webp'
  );
  console.log(`Cover uploaded: ${coverUrl}\n`);

  let comicId;
  const existing = await pool.query('SELECT id FROM comics WHERE title = $1', ['Eternally Regressing Knight']);
  if (existing.rows.length) {
    ...
    `, [
      ...
      'Unknown',
      'Unknown',
      'A knight who fell in battle is sent back in time again and again. Each regression he grows stronger, inching toward the future where he finally wins.',
      coverUrl,
      'Ongoing',
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/eternally-regressing-knight-ch${chNum}-${Date.now()}.pdf`;
