// Uploads "The Swordmaster's Youngest Son" to the live site (R2 + Supabase)
...
const SOURCE_DIR = "D:/The Swordmaster's Youngest Son";
const COVER_FILE = 'lhsapcsowumb1.jpg';
...
//   "[WF] [Ch-1] The Swordmaster s Son.pdf"
//   "[MC] [Ch-161] Swordmaster's Youngest Son.pdf"
//   "[Ch-169] Youngest Son of the Sword [@Manga_Cruise].pdf"
function parseChapterNum(filename) {
  let m = filename.match(/\[WF\]\s*\[Ch-(\d+(?:\.\d+)?)\]/i);
  if (m) return parseFloat(m[1]);
  m = filename.match(/\[MC\]\s*\[Ch-(\d+(?:\.\d+)?)\]/i);
  if (m) return parseFloat(m[1]);
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/swordmasters-youngest-son-cover-${Date.now()}.jpg`,
    'image/jpeg'
  );
  console.log(`Cover uploaded: ${coverUrl}\n`);

  let comicId;
  const existing = await pool.query("SELECT id FROM comics WHERE title = 'The Swordmaster''s Youngest Son'");
  if (existing.rows.length) {
    ...
    `, [
      ...
      coverUrl,
      'Ongoing',
      JSON.stringify(['Action', 'Comedy', 'Romance', 'Fantasy', 'Martial Arts', 'Manhwa', 'Reincarnation']),
      0
    ]);
    comicId = r.rows[0].id;
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/swordmasters-youngest-son-ch${chNum}-${Date.now()}.pdf`;
  ...
  console.log(`View at: https://mangvault.com/the-swordmasters-youngest-son`);
  await pool.end();
}
