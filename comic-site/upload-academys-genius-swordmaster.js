...
const SOURCE_DIR = "E:\\New folder\\Telegram Desktop\\Academy's Genius Swordmaster";
const COVER_FILE = 'academys-genius-swordmaster.38ec68-400.webp';
...
// Handles: "[CH - 010] Academy's Genius Swordmaster.pdf" and "[CH- 130] ..." variants
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/academys-genius-swordmaster-cover-${Date.now()}.webp`,
    'image/webp'
  );
  console.log(`Cover uploaded: ${coverUrl}\n`);

  let comicId;
  const existing = await pool.query("SELECT id FROM comics WHERE title = $1", ["Academy's Genius Swordmaster"]);
  if (existing.rows.length) {
    ...
    `, [
      ...
      'Unknown',
      'Unknown',
      "A genius swordmaster enrolls at the continent's most prestigious academy, hiding his true abilities while mastering all forms of combat.",
      coverUrl,
      'Ongoing',
      JSON.stringify(['Action', 'Fantasy', 'School Life', 'Manhwa']),
      0,
    ]);
    comicId = r.rows[0].id;
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/academys-genius-swordmaster-ch${chNum}-${Date.now()}.pdf`;
