// Uploads "The Boss's Daughter" to the live site (R2 + Supabase)
...
const SOURCE_DIR = "C:/Users/Administrator/Downloads/Telegram Desktop/The Boss's daughter";
...
async function main() {
  ...
  const coverPath = path.join(SOURCE_DIR, 'Wallpaper.jpg');
  console.log('Uploading cover image...');
  const coverUrl = await uploadFile(coverPath, `covers/bossdaughter-cover-${Date.now()}.jpg`, 'image/jpeg');
  console.log(`Cover uploaded: ${coverUrl}\n`);

  // Get or create the comic
  let comicId;
  const existing = await pool.query("SELECT id FROM comics WHERE title = 'The Boss''s Daughter'");
  if (existing.rows.length) {
    ...
    `, [
      ...
      'Unknown',
      'Unknown',
      "A captivating story about the boss's daughter and the secrets that surround her world.",
      coverUrl,
      'Ongoing',
  ...
  // Build chapter map — skip duplicates marked with "(2)" etc.
  const chapterMap = new Map();
  for (const file of allFiles) {
    const num = parseChapterNum(file);
    if (!num) { console.log(`  SKIP (no chapter number): ${file}`); continue; }
    if (/\(\d+\)/.test(file)) { console.log(`  SKIP (duplicate): ${file}`); continue; }
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/bossdaughter-ch${chNum}-${Date.now()}.pdf`;
