// Uploads "Eleceed" to the live site (R2 + Supabase)
...
const SOURCE_DIR = 'C:/Users/Administrator/Downloads/Telegram Desktop/Eleceed';
const COVER_FILE = 'photo_2026-04-05_19-32-28.jpg';
...
// Handles:
//   "Ch - 271 Eleceed.pdf"                    → 271
//   "Chapter 301 - Eleceed.pdf"               → 301
//   "Eleceed - Chapter 1 @Manga_LightN.pdf"   → 1
//   "Eleceed - Chapter 209.pdf"               → 209
//   "[CH] [389] Eleceed.pdf"                  → 389
//   "C•313 Eleceed.pdf"                       → 313
function parseChapterNum(filename) {
  // "Ch - 271 Eleceed.pdf"
  const m1 = filename.match(/^Ch\s*-\s*(\d+(?:\.\d+)?)/i);
  if (m1) return parseFloat(m1[1]);
  // "Chapter 301 - Eleceed.pdf"
  const m2 = filename.match(/^Chapter\s+(\d+(?:\.\d+)?)/i);
  if (m2) return parseFloat(m2[1]);
  // "Eleceed - Chapter NNN ..." or "Eleceed - Chapter NNN.pdf"
  const m3 = filename.match(/^Eleceed\s*-\s*Chapter\s+(\d+(?:\.\d+)?)/i);
  if (m3) return parseFloat(m3[1]);
  // "[CH] [NNN] Eleceed.pdf"
  const m4 = filename.match(/^\[CH\]\s*\[(\d+(?:\.\d+)?)\]/i);
  if (m4) return parseFloat(m4[1]);
  // "C•313 Eleceed.pdf" (unicode bullet)
  const m5 = filename.match(/^C[•.]\s*(\d+(?:\.\d+)?)/i);
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/eleceed-cover-${Date.now()}.jpg`,
    'image/jpeg'
  );
  console.log(`Cover uploaded: ${coverUrl}\n`);

  let comicId;
  const existing = await pool.query("SELECT id FROM comics WHERE title = 'Eleceed'");
  if (existing.rows.length) {
    ...
    `, [
      ...
      coverUrl,
      'Releasing',
      JSON.stringify(['Action', 'Comedy', 'Supernatural', 'Manhwa']),
      0
    ]);
    comicId = r.rows[0].id;
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/eleceed-ch${chNum}-${Date.now()}.pdf`;
