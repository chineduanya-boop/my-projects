// Uploads "Solo Leveling" to the live site (R2 + Supabase)
...
const SOURCE_DIR = 'C:/Users/Administrator/Downloads/Telegram Desktop/Solo Leveling';
const COVER_FILE = 'hbg-title-solo-leveling-vol-8-comic-215.webp';
...
// Handles: "Ch - 00", "CH - 194 - Side Story 15", "Chapter 180 - Side Story 1"
// Skips: "Chapter spoiler" (no number follows)
function parseChapterNum(filename) {
  const m = filename.match(/(?:chapter\s+|ch\s*-\s*)(\d+(?:\.\d+)?)/i);
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/solo-leveling-cover-${Date.now()}.webp`,
    'image/webp'
  );
  ...
  let comicId;
  const existing = await pool.query("SELECT id FROM comics WHERE title = 'Solo Leveling'");
  if (existing.rows.length) {
    ...
    `, [
      ...
      'Chugong',
      'Dubu (Redice Studio)',
      'In a world where hunters—humans with supernatural abilities—must battle deadly monsters to protect humanity, Sung Jinwoo is the weakest of them all. One fateful day he encounters a double dungeon and is given a second chance at life with a mysterious system that only he can see.',
      coverUrl,
      'Completed',
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/solo-leveling-ch${chNum}-${Date.now()}.pdf`;
