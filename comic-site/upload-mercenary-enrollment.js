// Uploads "Mercenary Enrollment" to the live site (R2 + Supabase)
...
const SOURCE_DIR = 'C:/Users/Administrator/Downloads/Telegram Desktop/MERCENARY ENROLLMENT';
const COVER_FILE = 'MERCENARY ENROLLMENT.jpg';
...
// Handles two naming patterns in this folder:
//   "Ch - 000 Mercenary Enrollment.pdf"       → 0
//   "[CH] [261] Mercenary Enrollment @...pdf"  → 261
function parseChapterNum(filename) {
  // Pattern 1: [CH] [261] ...
  let m = filename.match(/\[CH\]\s*\[(\d+(?:\.\d+)?)\]/i);
  if (m) return parseFloat(m[1]);
  // Pattern 2: Ch - 000 ...
  m = filename.match(/Ch\s*-\s*(\d+(?:\.\d+)?)/i);
  if (m) return parseFloat(m[1]);
  return null;
}
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/mercenary-enrollment-cover-${Date.now()}.jpg`,
    'image/jpeg'
  );
  ...
  let comicId;
  const existing = await pool.query("SELECT id FROM comics WHERE title = 'Mercenary Enrollment'");
  if (existing.rows.length) {
    ...
    `, [
      ...
      'YC',
      'Mingyeon',
      'Yu I-Jin was the sole survivor of a plane crash when he was little. After becoming a mercenary to survive for 10 years, he returns to his family in his hometown. Now enrolled in high school, the strongest mercenary must navigate the simplest of battles — school life.',
      coverUrl,
      'Ongoing',
      JSON.stringify(['Action', 'Drama', 'Manhwa']),
      1,
    ]);
    comicId = r.rows[0].id;
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/mercenary-enrollment-ch${chNum}-${Date.now()}.pdf`;
  ...
  console.log(`View at: https://mangvault.com/mercenary-enrollment`);
  await pool.end();
}
