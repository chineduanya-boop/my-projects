// Uploads "Reincarnation of the Suicidal Battle God" to the live site (R2 + Supabase)
...
const SOURCE_DIR = 'D:/Reincarnation of the Suicidal Battle God';
const COVER_FILE = 'photo_2026-04-11_16-57-46.jpg';
...
// Handles: "01 Reincarnation of the Suicidal Battle God.pdf", "100 Reincarnation..."
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/suicidal-battle-god-cover-${Date.now()}.jpg`,
    'image/jpeg'
  );
  ...
  let comicId;
  const existing = await pool.query("SELECT id FROM comics WHERE title = $1", ['Reincarnation of the Suicidal Battle God']);
  if (existing.rows.length) {
    ...
    `, [
      ...
      coverUrl,
      'Releasing',
      JSON.stringify(['Action', 'Fantasy', 'Manhwa', 'Dungeon', 'Reincarnation']),
      0
    ]);
    comicId = r.rows[0].id;
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/suicidal-battle-god-ch${chNum}-${Date.now()}.pdf`;
