// Uploads "Solo Leveling: Ragnarok" to the live site (R2 + Supabase)
...
const SOURCE_DIR = 'C:/Users/Administrator/Downloads/Telegram Desktop/Solo leveling Rangnorok';
const COVER_FILE = 'hbg-title-solo-leveling-ragnarok-vol-1-comic-5.webp';
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/solo-leveling-ragnarok-cover-${Date.now()}.webp`,
    'image/webp'
  );
  console.log(`Cover uploaded: ${coverUrl}\n`);

  // Get or create the comic record
  let comicId;
  const existing = await pool.query("SELECT id FROM comics WHERE title = 'Solo Leveling: Ragnarok'");
  if (existing.rows.length) {
    ...
    `, [
      ...
      'Gina',
      'Redice Studio',
      'The sequel to Solo Leveling. Set years after Sung Jinwoo saved the world, his son Sung Suho inherits his legendary shadow powers and faces a new wave of threats threatening humanity.',
      coverUrl,
      'Ongoing',
  ...
  // Collect and sort all PDF chapters
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/solo-leveling-ragnarok-ch${chNum}-${Date.now()}.pdf`;
