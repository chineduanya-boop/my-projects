// Uploads "Absolute Sword Sense" to the live site (R2 + Supabase)
...
const SOURCE_DIR = 'D:/Absolute Sword Sense';
const COVER_FILE = 'photo_2026-04-11_13-05-22.jpg';
...
// Handles: "[CH - 100] Absolute Sword Sense.pdf"
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/absolute-sword-sense-cover-${Date.now()}.jpg`,
    'image/jpeg'
  );
  console.log(`Cover uploaded: ${coverUrl}\n`);

  let comicId;
  const existing = await pool.query("SELECT id FROM comics WHERE title = 'Absolute Sword Sense'");
  if (existing.rows.length) {
    ...
    `, [
      ...
      coverUrl,
      'Ongoing',
      JSON.stringify(['Action', 'Adventure', 'Fantasy', 'Martial Arts', 'Manhwa', 'Time Travel']),
      0
    ]);
    comicId = r.rows[0].id;
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/absolute-sword-sense-ch${chNum}-${Date.now()}.pdf`;
  ...
  console.log(`View at: https://mangvault.com/absolute-sword-sense`);
  await pool.end();
}
