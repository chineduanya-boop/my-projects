// Uploads "The Hero Returns" to the live site (R2 + Supabase)
...
const SOURCE_DIR = 'C:/Users/Administrator/Downloads/Telegram Desktop/Uploaded/The Hero Returns';
const COVER_FILE = 'The_Hero_Returns.webp';
...
// Handles: "[001] [MW] The Hero Returns [@Manhwa_Weebs].pdf"
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/hero-returns-cover-${Date.now()}.webp`,
    'image/webp'
  );
  ...
  let comicId;
  const existing = await pool.query("SELECT id FROM comics WHERE title = 'The Hero Returns'");
  if (existing.rows.length) {
    ...
    `, [
      ...
      coverUrl,
      'Ongoing',
      JSON.stringify(['Action', 'Adventure', 'Dungeons', 'Fantasy', 'Shounen', 'Time Travel', 'Transmigration']),
      0
    ]);
    comicId = r.rows[0].id;
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/hero-returns-ch${chNum}-${Date.now()}.pdf`;
  ...
  console.log(`View at: https://mangvault.com/the-hero-returns`);
  await pool.end();
}
