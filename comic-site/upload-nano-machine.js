// Uploads "Nano Machine" to the live site (R2 + Supabase)
...
const SOURCE_DIR = 'C:/Users/Administrator/Downloads/Telegram Desktop/Uploaded/Nano Machine';
const COVER_FILE = 'Nanomachine.webp';
...
// Handles: "[ME] Nano Machine [173] @MangaEstate.pdf"
function parseChapterNum(filename) {
  const m = filename.match(/\[ME\]\s*Nano Machine\s*\[(\d+(?:\.\d+)?)\]/i);
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/nano-machine-cover-${Date.now()}.webp`,
    'image/webp'
  );
  ...
  let comicId;
  const existing = await pool.query("SELECT id FROM comics WHERE title = 'Nano Machine'");
  if (existing.rows.length) {
    ...
    `, [
      ...
      coverUrl,
      'Ongoing',
      JSON.stringify(['Action', 'Adventure', 'Fantasy', 'Sci-Fi', 'Martial Arts', 'Manhwa']),
      0
    ]);
    comicId = r.rows[0].id;
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/nano-machine-ch${chNum}-${Date.now()}.pdf`;
  ...
  console.log(`View at: https://mangvault.com/nano-machine`);
  await pool.end();
}
