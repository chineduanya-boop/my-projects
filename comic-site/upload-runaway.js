...
const SOURCE_DIR = 'C:/Users/Administrator/Downloads/Telegram Desktop/Run Away';
...
function parseChapterNum(filename) {
  const m = filename.match(/chapter\s+(\d+(?:\.\d+)?)/i);
...
async function main() {
  ...
  // Upload cover image
  const coverPath = path.join(SOURCE_DIR, 'cover image.jpg');
  console.log('Uploading cover image...');
  const coverUrl = await uploadFile(coverPath, `covers/runaway-cover-${Date.now()}.jpg`, 'image/jpeg');
  console.log(`Cover uploaded: ${coverUrl}\n`);

  // Get or create comic
  let comicId;
  const existing = await pool.query("SELECT id FROM comics WHERE title = 'Run Away'");
  if (existing.rows.length) {
    ...
    `, [
      ...
      'Unknown',
      'Unknown',
      'A gripping story of escape, survival, and finding freedom against all odds.',
      coverUrl,
      'Ongoing',
      JSON.stringify(['Drama', 'Romance', 'Manhwa']),
      1
    ]);
    comicId = r.rows[0].id;
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/runaway-ch${chNum}-${Date.now()}.pdf`;
