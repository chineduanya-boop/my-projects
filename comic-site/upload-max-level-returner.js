// Uploads "The Max Level Returner" to the live site (R2 + Supabase)
...
const SOURCE_DIR = 'C:/Users/Administrator/Downloads/Telegram Desktop/The Max Level Returner';
const COVER_FILE = 'photo_2022-01-18_00-55-55.jpg';
...
// Handles:
//   "Chapter.000-1-@Manga_loverz.pdf"  → 0
//   "Chapter.147-@Manga_loverz.pdf"    → 147
//   "Chapter 163-@Manga_Loverz.pdf"    → 163
//   "166 - @Manga_Loverz.pdf"          → 166
function parseChapterNum(filename) {
  // "Chapter.NNN-..." or "Chapter NNN-..."
  const m1 = filename.match(/^Chapter[.\s]+(\d+(?:\.\d+)?)/i);
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/max-level-returner-cover-${Date.now()}.jpg`,
    'image/jpeg'
  );
  console.log(`Cover uploaded: ${coverUrl}\n`);

  let comicId;
  const existing = await pool.query("SELECT id FROM comics WHERE title = 'The Max Level Returner'");
  if (existing.rows.length) {
    ...
    `, [
      ...
      '',
      '',
      'A total of 120 million people disappeared from all over the world. [You have successfully completed the final quest, and have been given the reward: Return.] In 22 years, Yoon Sang-Hyuk was finally able to beat the most atrocious survival game. The man who became the One Man Army, and the strongest out of all of the other players, he had finally returned back to reality. And not just that, he came back with a reward that nobody had ever received before. This is the start of the Max Level Returner!',
      coverUrl,
      'Releasing',
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/max-level-returner-ch${chNum}-${Date.now()}.pdf`;
