// Uploads "The Beginning After The End (TBATE)" to the live site (R2 + Supabase)
...
const SOURCE_DIR = 'C:/Users/Administrator/Downloads/Telegram Desktop/THE BEGINNING AFTER THE END  TBATE';
const COVER_FILE = 'photo_2049-02-03_23-15-00.jpg';
...
// Handles:
//   "[MO] [001] The Beginning After The End @Manhwa_Ocean.pdf"  → 1
//   "[189] The Beginning After The End @Manhwa_Garden.pdf"      → 189
function parseChapterNum(filename) {
  // Pattern: [MO] [NNN] ...
  const mo = filename.match(/^\[MO\]\s*\[(\d+)\]/);
  if (mo) return parseFloat(mo[1]);

  // Pattern: [NNN] ...
  const plain = filename.match(/^\[(\d+)\]/);
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/tbate-cover-${Date.now()}.jpg`,
    'image/jpeg'
  );
  ...
  const existing = await pool.query(
    "SELECT id FROM comics WHERE title = 'The Beginning After The End'"
  );
  ...
  for (const file of allFiles) {
    ...
    // If duplicate chapter number, prefer the [MO] source (lower chapter range)
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/tbate-ch${chNum}-${Date.now()}.pdf`;
