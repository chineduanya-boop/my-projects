...
const SOURCE_DIR = 'E:\\New folder\\Telegram Desktop\\Star-Embracing Swordmaster';
const COVER_FILE = 'star-embracing-swordmaster.6b1222-400.webp';
...
// Handles: "[001] Star Embracing Swordsmaster @Manhwa_Garden.pdf"
...
async function main() {
  ...
  const coverUrl = await uploadFile(
    coverPath,
    `covers/star-embracing-swordmaster-cover-${Date.now()}.webp`,
    'image/webp'
  );
  console.log(`Cover uploaded: ${coverUrl}\n`);

  let comicId;
  const existing = await pool.query('SELECT id FROM comics WHERE title = $1', ['Star-Embracing Swordmaster']);
  if (existing.rows.length) {
    ...
    `, [
      ...
      'Unknown',
      'Unknown',
      'A legendary swordmaster who reached the pinnacle of swordsmanship reincarnates and walks the path to become the greatest sword master under the stars.',
      coverUrl,
      'Ongoing',
  ...
  for (const [chNum, file] of sorted) {
    ...
    const r2Key = `pdfs/star-embracing-swordmaster-ch${chNum}-${Date.now()}.pdf`;
